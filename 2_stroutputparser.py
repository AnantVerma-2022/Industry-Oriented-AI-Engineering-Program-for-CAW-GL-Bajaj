from langchain_groq import ChatGroq
from dotenv import load_dotenv
from langchain_core.prompts import PromptTemplate
from langchain_core.output_parsers import StrOutputParser
import os
load_dotenv()

os.environ["GROQ_API_KEY"] = os.getenv("GROQ_API_KEY")

model = ChatGroq(
    model="llama-3.1-8b-instant"
)
# 1st prompt -> detailed report
# 1st prompt to generate a detailed report
template1 = PromptTemplate(
    template='Write an essay on the {topic}',
    input_variables=['topic']
)

# 2nd prompt to generate a summary
template2 = PromptTemplate(
    template='Write summary in 5 bullets on the following text. /n {text}',
    input_variables=['text']
)

# create an instance of the StrOutputParser
str_output_parser = StrOutputParser()

# chain the prompts together and use the StrOutputParser to parse the output of the first prompt before passing it to the second prompt
chain = template1 | model | str_output_parser | template2 | model | str_output_parser

result = chain.invoke({'topic':'Dewali'})

print(result)